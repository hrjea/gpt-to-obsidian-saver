...
For every ordinary Markdown, HTML learning, generated detailed-Markdown, or explicitly approved partial save, version 1.5.52 asks for separate explicit ChatGPT Share consent before creating the Obsidian note. Continue can create, update, reuse, or copy a ChatGPT-hosted response link; only when response Share is missing may a separately disclosed whole-conversation fallback be used. Cancel requests no clipboard permission, clicks no Share control, and creates no note. The strictly validated URL is added as a supplemental link to the local note; it does not make the locally captured body a rich-app remote reference.

For a supported interactive-app response, the extension retains its specialized, explicitly consented ChatGPT Share flow. In that mode the strictly validated URL is the remote reference for behavior that was not copied locally, so the note is not an offline copy of the app. A whole-conversation fallback is broader than a response link and requires its own consent in either flow.
...
The optional `clipboardRead` permission may be requested only during an explicitly approved ordinary supplemental-link or specialized remote-reference Share flow. Immediately before making that synchronous request from the consent action, the extension revalidates its runtime and the applicable selected-turn proof: ordinary attempts check the frozen selected assistant, nearest earlier user, any ordered intervening-assistant provenance, route, and exact Share scope/control; hydrated rich-app attempts retain their stronger conversation/app/scroller proof. A mismatch stops before Chrome is asked for permission, except that one strictly verified missing-only virtualization window skips the optional permission with zero permission requests and defers its bounded recovery until after consent returns approved. Recovery never scrolls inside the consent callback, and the skipped permission is not requested automatically afterward. The extension then keeps clipboard reading disabled and may use its own empty manual URL field. Consent, the request, the read, and use of the value are bounded to the approved attempt. It reads at most one clipboard value only after both permission and a fresh, strict copy-success signal for the current action. Chrome's granted optional permission may persist until the user or browser revokes it; the extension does not automatically remove it. Raw clipboard/manual values are not logged or persisted. A value enters the note only after strict ChatGPT share-URL validation.
...
The explicit ChatGPT Share flows described above are the only current remote-sharing exception. They use ChatGPT's own visible UI and service, not a developer-operated server or private ChatGPT API. Version 1.5.52 adds no required or optional permission, telemetry, remote note store, or new runtime/Native payload field. If a response Create/Update action or a whole-conversation dialog action/instant-copy occurred and the later Obsidian save fails, the link may remain active; the extension warns the user and does not automatically revoke it. Merely reusing or copying an existing response link does not by itself enter that persistence-warning state.
